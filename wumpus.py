...
import wumpus_environment as e
...
class WumpusWorldScenario(object):
    ...
    def __init__(self, layout_file=None, agent=None, objects=None,
        ...
        """
        if agent != None and not isinstance(agent, e.Explorer):
    ...
    def build_world(self, width, height, entrance, agent, objects):
        ...
        """
        env = e.WumpusEnvironment(width, height, entrance)
        if self.trace:
            agent = e.wumpus_environment.TraceAgent(agent)
...
def world_scenario_manual_from_layout(layout_filename):
    ...
    """
    return WumpusWorldScenario(layout_file = layout_filename,
                               agent = with_manual_program(e.Explorer(heading='north', verbose=True)),
                               trace=False)
...
def wscenario_4x4():
    return WumpusWorldScenario(agent = e.Explorer(heading='north', verbose=True),
                               objects = [(e.Wumpus(),(1,3)),
                                          (e.Pit(),(3,3)),
                                          (e.Pit(),(4,4)),
                                          (e.Pit(),(3,1)),
                                          (e.Gold(),(2,3))],
                               width = 5, height = 5, entrance = (1,2),
                               trace=False)
